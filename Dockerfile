...
COPY . .
RUN npm install
RUN sed -i 's/DBUSER:'.*'/DBUSER:test/' /usr/src/app/node_modules/outlawdesigns.io.noderecord/config.js
RUN sed -i 's/DBHOST:'.*'/DBHOST:localhost/' /usr/src/app/node_modules/outlawdesigns.io.noderecord/config.js
RUN sed -i 's/DBPASS:'.*'/DBPASS:test/' /usr/src/app/node_modules/outlawdesigns.io.noderecord/config.js
EXPOSE 9690
